// VS1053b MIDI - Hailstone sequence music //
...
void loop() {
  ...
  while(n != 1) {
    ...
    uint8_t note0 = constrain(n, 24, 48);
    uint8_t note1 = constrain(n, 36, 64);
    uint8_t note2 = constrain(n, 24, 64);
    uint8_t note3 = constrain(n, 36, 64);
    uint8_t note4 = constrain(n, 24, 64);
    uint8_t drum = constrain(n, 27, 68);

    note_on(0, 0, note0, 36 + rand()%64);
    note_on(1, 13, note1, 36 + rand()%64);
    note_on(2, 23, note2, 36 + rand()%64);
    note_on(3, 24 + rand()%64, note3, 36 + rand()%64);
    note_on(4, 90, note4, 36 + rand()%64);

    if(drums = !drums) note_on(9, 0, 35, 96 + rand()%24);
    
    note_on(9, 0, drum, 36 + rand()%56);
